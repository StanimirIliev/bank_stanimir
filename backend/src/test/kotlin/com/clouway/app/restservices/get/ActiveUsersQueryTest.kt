...
import com.clouway.app.adapter.http.get.ActiveUsersRoute
...
class ActiveUsersQueryTest {
    ...
    fun setUp() {
        ...
        port(port)
        get("/v1/activeUsers", ActiveUsersRoute(
                sessionRepository,
                Logger.getLogger(ActiveUsersQueryTest::class.java)
        ))
        awaitInitialization()
    }
    ...
    fun getActiveUsersCountAsRegisteredUserJsonData() {
        ...
        val request = HttpGet("http://$domain:$port/v1/activeUsers")
        ...
        assertThat(responseContent, `is`(equalTo("{\"activeUsers\":1}")))
    }
